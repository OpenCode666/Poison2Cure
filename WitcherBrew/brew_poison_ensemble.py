...
args = forest.options_gru_ensble_M(num_ensemble=5,trial_idx=5)
...
args.log_file_dir = current_directory + '/log_file_ens' # log save path
parent_directory = os.path.abspath(os.path.join(os.getcwd(), ".."))
args.pretrained_dir = current_directory + '/example/Models_M_FT'
args.pretrained_dir_true = current_directory + '/example/Models_Truth_FT'
args.net = ['raw_gru']
...
if __name__ == "__main__":
    ...
    if args.flag_clean_vruns:
        ...
        stats_results = model.validate_on_true(data, None)
    ...
    brew_time = time.time()
    save_draw_poison_delta_distribution(poison_delta, poison_loss_value,
                                        args.log_file_dir)   # passenger loss during brewing poison

    if args.vruns > 0:
        stats_results = model.validate_on_true(data, poison_delta)
    else:
        stats_results = None
    test_time = time.time()
    ...
    time_path = current_directory + '/log_file_ens/timestamps.mat'
