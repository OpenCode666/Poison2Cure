...
class _VictimBase:
    ...
    def validate_on_true(self, kettle, poison_delta):
        run_stats = list()
        for runs in range(self.args.vruns):
            self.initialize()
            if self.args.pretrained:
                self.load_pretrained(self.args.pretrained_dir) 
            print('Model reinitialized to random seed.')
            run_stats.append(self._iterate_valid(kettle, poison_delta=poison_delta))
    ...
    def _initialize_model(self, model_name):
        ...
        model = get_model(model_name, self.args.dataset, pretrained=self.args.pretrained)    # build an initial model
        # Define training routine
        defs = training_strategy(model_name, self.args)
        criterion = torch.nn.CrossEntropyLoss(reduction='sum')
        optimizer, scheduler = get_optimizers(model, self.args, defs)
        weight_FT_PT = defs.weight_FT_PT

        return model, defs, criterion, optimizer, scheduler, weight_FT_PT
    ...
    def _step(self, kettle, poison_delta, loss_fn, epoch, stats, model, defs, criterion, optimizer, scheduler):
        ...
        run_step(kettle, poison_delta, loss_fn, epoch, stats, model, defs, criterion, optimizer, scheduler, self.weight_FT_PT)
