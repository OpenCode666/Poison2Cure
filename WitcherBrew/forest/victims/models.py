...
def get_model(model_name, dataset_name, pretrained=True):
    """Retrieve an appropriate architecture."""

    if 'data_for_poison' in dataset_name:
        DATA_VEC_LEN = 370  # Length of the data vector
        NUM_CLASS = 10
        if 'raw_gru' in model_name:
            from modules.GRU_model import GRUClassifier
            model = GRUClassifier(DATA_VEC_LEN, NUM_CLASS)
        else:
            raise ValueError(f'Model {model_name} not found')
    if 'data_for_ensemble' in dataset_name:
        DATA_VEC_LEN = 370  # Length of the data vector
        NUM_CLASS = 10
        if 'raw_gru' in model_name:
            feat_len = model_name[1]
            layer_num = model_name[2]
            from modules.GRU_model import GRUClassifier_surrogate
            model = GRUClassifier_surrogate(DATA_VEC_LEN, NUM_CLASS, feat_len, layer_num)
        else:
            raise ValueError(f'Model {model_name} not found')
